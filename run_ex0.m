...
% RUN_DISTRIBUTION_ANALYSIS Analyzes and compares the analytical PDFs of three 
% quantitative finance distributions (MA, AB, GL).
%
% INPUT:
%   check_plot : boolean flag (default false). If true, it runs a 1M 
%                Monte Carlo simulation for the AB model and plots the 
%                empirical vs analytical PDF.
    ...
    %% 5. AB Monte Carlo Simulation vs Analytical PDF (Optional)
    if check_plot
        numSim = 1e6;
    ...
    %% 7. PDF Comparison (Semi-Log Scale)
    figure('Color', 'w', 'Position', [100, 100, 900, 600]);
    hold on;
    grid on;
    box on;
