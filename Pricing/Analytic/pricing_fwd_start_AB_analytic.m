function price = pricing_fwd_start_AB_analytic(params, sc_T1, sc_T2, df, K2, ...
                                               forward, fwd_factor, M, dz)
%PRICING_FWD_START_AB_ANALYTIC  Semi-analytic forward-start price under the
%   Additive Bachelier (AB) model.
%
%   Prices  df * E[ ( S(T2) - K2*F(T1,T2) )_+ ]  by Lewis-FFT inversion of the
%   Lemma-2 (Forward.pdf) increment CF cf_increment_AB, conditioning on the
...
%   price      : 1 x numel(K2) row of forward-start prices
    ...
    price = fwd_start_LA_analytic(@cf_increment_AB, 'AB', params, sc_T1, sc_T2, ...
