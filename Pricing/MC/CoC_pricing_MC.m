function [price, CI, ft1, call_price_t1] = CoC_pricing_MC(params, scale_factor, N_sim, M, dz, N_grid, forward, K1, K2, discount_factors, model, verbose)
...
%   verbose         - (optional, default true) print the per-model diagnostics.
%
% OUTPUTS:
...
%   call_price_t1   - (vector) inner call prices at t1 for each path
    ...
    if nargin < 12 || isempty(verbose), verbose = true; end
    ...
    if verbose, fprintf('  [1/3] Simulating ft1...\n'); end
    switch model
        case 'MA'
        ...
        case 'GL'
            ...
            if verbose, fprintf('        z_grid: [%.4f, %.4f]  (%d points)\n', z_grid(1), z_grid(end), numel(z_grid)); end
