...
%   The inner put value at t1 is obtained from the inner call by put-call
%   parity, and the compound payoff max(K1 - P_t1, 0) is discounted to t0.
%
% INPUTS:
...
%   put_price_t1    - (vector) inner put prices at t1 for each path
    ...
    if opts.verbose
        fprintf('\n=================================================================\n');
        fprintf('  POP PRICING MC — Model: %s\n', model);
        fprintf('=================================================================\n');
    ...
    % Vectorized broadcast: put_price_t1 is forced to (N_sim x 1), K1 to (1 x N_k)
    % Resulting matrix is (N_sim x N_k)
    put_col = put_price_t1(:);
    K1_row  = K1(:)';
    
    payoffs = max(K1_row - put_col, 0);
